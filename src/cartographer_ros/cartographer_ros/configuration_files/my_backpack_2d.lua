...
options = {
  ...
  published_frame = "base_link", --一个用来发布姿态的子级坐标系，主要用来校正tracking_frame；
  odom_frame = "odom",  --仅当provide_odom_frame=true时可以使用，位置关系是map_frame->odom_frame->published_frame，被用来发布局部的slam激光里程计结果，不用于回环检测
  provide_odom_frame = false,
  publish_frame_projected_to_2d = false,  --如果为true，则发布的姿态published_frame将会是纯2D姿态，没有横滚，俯仰，偏转角。
  use_pose_extrapolator = true,
  use_odometry = false, --如果为true，将会订阅“odom”这个话题，消息类型是nav_msgs/Odometry。这种情况下，必须提供里程计数据，并且这些数据将会包含进slam
  ...
  --num_multi_echo_laser_scaframens = 0, --订阅多线激光雷达话题的数量
